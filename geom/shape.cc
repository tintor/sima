...
Shape::Shape(const Mesh3d& mesh, /*in/out*/ pose3& position) {
    if (is_valid(mesh) != Validity::OK) throw runtime_error("invalid mesh");
    ...
    FOR(i, 3) {
        ...
        if (edge_angle(a, b, c, third_vertex[segment3(b, a)]) < PI) {
            if (lexicographical_less(a, b)) m_convex_edges.push_back(segment3(a, b));
        } else {
    ...
    std::sort(m_mesh, [](const triangle3& p, const triangle3& q) { return p.squared_area_x4() > q.squared_area_x4(); });
    // Sort convex edges by decreasing length
    std::sort(m_convex_edges,
              [](const segment3& p, const segment3& q) { return squared(p.a - p.b) > squared(q.a - q.b); });
    // Sort convex vertices by decreasing distance from center
    std::sort(m_convex_vertices, [](double3 p, double3 q) { return squared(p) > squared(q); });
}
...
real signed_distance(double3 v, const Shape& shape) {
    // Can't just take the sdist sign of min_dist as two faces that share the edge can have the same dist but different
    // sgn(sdist)
    real min_dist = std::numeric_limits<real>::max(), max_sdist = 0;
    constexpr real eps =
        ContactEpsilon;  // ??? arbitrary, need to account for rounding errors when computing distance(vertex, face)
    FOR_EACH(face, shape.faces()) {
        ...
        if (plane::sign(*a, *b, *a + p.normal, v) > 0) {
            ...
            if (dist > min_dist + eps) goto next;
        ...
        sdist = p.distance(v);
        dist = abs(sdist);
        if (dist > min_dist + eps) continue;
    rest:
        if (dist < min_dist) {
            if (dist + eps < min_dist) max_sdist = sdist;
            min_dist = dist;
        }
        if (abs(sdist) > abs(max_sdist)) max_sdist = sdist;
    next:;
    }
...
real squared_distance_segment_origin(const segment3& p) {
    double3 d = p.b - p.a;
    real t = -dot(p.a, d);
    if (t <= 0) return squared(p.a);
    real dd = squared(d);
    if (t >= dd) return squared(p.b);
...
bool intersects_edge_interior(const segment3& edge, const Shape& shape) {
    ...
    if (squared_distance_segment_origin(edge) > squared(shape.sphere_radius())) return false;

    // Quick bounding box check
    // if (!is_edge_intersecting_box())
    //	return false;
    if (shape.is_box()) return true;
...
bool are_oriented_boxes_intersecting(const pair<double3, double3>& box_a, const pose3& pos_a,
...
bool intersects(const Shape& shape_a, const pose3& pos_a, const Shape& shape_b, const pose3& pos_b) {
    ...
    if (intersects(pos_b.to_local(pos_a.to_global(vertex_a)), shape_b)) return true;
    FOR_EACH(vertex_b, shape_b.convex_vertices())
    if (intersects(pos_a.to_local(pos_b.to_global(vertex_b)), shape_a)) return true;

    // Check all edges of one against the other
    FOR_EACH(edge_a, shape_a.convex_edges())
    if (intersects_edge_interior(pos_b.to_local(pos_a.to_global(edge_a)), shape_b)) return true;
    FOR_EACH(edge_b, shape_b.convex_edges())
    if (intersects_edge_interior(pos_a.to_local(pos_b.to_global(edge_b)), shape_a)) return true;
...
bool is_vertex_triangle_contact(double3 p, const triangle3& m, Contact& /*out*/ contact) {
    ...
    FOR_EACH_EDGE(a, b, m)
    if (plane::sign(*a, *b, *a + normal, p) > 0) return false;

    // P is inside of triangle
    double3 nearest = p - normal * dot(normal, p - m.a);
    contact.squared_dist = squared(nearest - p);
    if (contact.squared_dist > squared(ContactEpsilon)) return false;
...
bool is_edge_edge_contact(const segment3& p, const segment3& q, Contact& /*out*/ contact) {
    ...
    if ((d >= tiny && d >= tiny * aa * bb && 0 <= s && s <= d && 0 <= t && t <= d) ||
        ...
        if (dot(P - Q, contact.normal) < 0) contact.normal = -contact.normal;
        return true;
    }
...
bool all_less_equal(double3 a, double3 b) { return a.x <= b.x && a.y <= b.y && a.z <= b.z; }

bool approx_intersects(double3 v, const Shape& shape) {
    return squared(v) <= squared(shape.sphere_radius()) && all_less_equal(shape.box().first, v) &&
           all_less_equal(v, shape.box().second);
}
...
vector<Contact> find_all_contacts(const Shape& shape_a, const pose3& pos_a, const Shape& shape_b, const pose3& pos_b) {
    ...
    FOR_EACH(vertex_a, shape_a.convex_vertices()) {
        ...
        if (!approx_intersects(vertex_a_local, shape_b)) continue;
    ...
    FOR_EACH(vertex_b, shape_b.convex_vertices()) {
        ...
        // TODO this approx check needs to be a little loose to allow for contacts
        if (!approx_intersects(vertex_b_local, shape_a)) continue;
    ...
    FOR_EACH(edge_a, shape_a.convex_edges()) {
        ...
        // TODO this approx check needs to be a little loose to allow for contacts
        if (!approx_intersects(pos_b.to_local(edge_a_global), shape_b)) continue;
        // TODO avoid transforming every edge in B
        FOR_EACH(edge_b, shape_b.convex_edges())
        if (is_edge_edge_contact(edge_a_global, pos_b.to_global(edge_b), /*out*/ contact)) contacts.push_back(contact);
    }
