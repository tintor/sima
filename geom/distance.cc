...
double Distance(sphere s, double3 v) { return max(0.0, length(s.center() - v) - s.radius()); }
double Distance(sphere a, sphere b) { return max(0.0, length(a.center() - b.center()) - a.radius() - b.radius()); }
...
vector<segment3> UniqueEdges(const xmesh3& ma) { THROW(not_implemented); }

inline segment3 ReverseIf(segment3 s, bool reverse) { return reverse ? s.reversed() : s; }

static double DistanceFaceVertexPairs(const xmesh3& ma, const xmesh3& mb, sphere sb, double min_distance, bool reverse,
                                      segment3* nearest) {
    for (const face& f : ma) {
        optional<plane> p;
        ...
        if (Distance(s, sb) < min_distance)
            for (const double3& v : mb.vertices())  // TODO faster if iterating over unique vertices
                if (squared(s.center() - v) < squared(min_distance + s.radius())) {
                    if (!p.has_value()) p = best_fit_plane(f.vertices());
                    if (abs(p->distance(v)) < min_distance && ClassifyFacePrismPoint(f, v, *p) <= 0) {
                        ...
                        if (d < min_distance) {
                            if (nearest) *nearest = ReverseIf(segment3(v - signed_distance * p->normal(), v), reverse);
                            min_distance = d;
                        }
                    }
                }
...
double Distance(cspan<double3> ma, cspan<double3> mb, sphere sa, sphere sb, segment3* nearest) {
    ...
    //      start comparing nearest pairs and maintain lower_bound and upper_bound until remaining vertices are far from
    //      plane
    ...
    for (double3 a : ma)
        if (squared(a - sb.center()) < squared(ds + sb.radius()))
            for (double3 b : mb) {
                ...
                if (e < ds) {
                    ds = e;
                    if (nearest) *nearest = segment3(a, b);
                }
            }
    return sqrt(ds);
}
