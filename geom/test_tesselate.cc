...
static polygon2 random_polygon(int size, RNG& rng) {
    ...
    while (points.size() < size) points.insert(uniform2(rng, -1000, 1000));
...
struct Setup {
    ...
    void read() {
        ...
        while (true) {
            ...
            if (!IsValid(poly)) THROW(runtime_error, "polygon not valid");
    ...
    void write() {
        ...
        for (auto i : range(3, 701)) {
            if (i % 16 == 0) print("prepare %s\n", i);
            polygon2 poly;
            while (!IsValid(poly)) poly = random_polygon(i, rng);
...
TEST_CASE("tesselate_verify", "[tesselate]") {
    for (const auto& poly : test_cases) {
        ...
        double tess_area = 0;
        for (auto m : tess) tess_area += area(m);
        ...
        for (auto e : Edges(poly)) poly_edges.insert(e);
