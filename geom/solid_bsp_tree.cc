...
void SolidBSPTree::evaluate_candidate(plane candidate, BuildData& data, const vector<uint>& mesh, uint* samples_begin,
                                      uint* samples_end, long& best_heuristic, Hist& best_hist, plane& best_candidate) {
    size_t psamples = 0;
    for (auto it = samples_begin; it != samples_end; it++)
        if (candidate.distance(data.samples[*it]) > 0) psamples += 1;
    size_t samples = samples_end - samples_begin;
    size_t nsamples = samples - psamples;
    if (psamples == 0 || nsamples == 0) return;
...
pair<uint, float> SolidBSPTree::build_internal(BuildData& data, const vector<uint>& mesh, uint* samples_begin,
                                               uint* samples_end) {
    const uint Outside = 0;
    const uint Inside = 0xFFFFFFFF;

    if (mesh.size() >= 1000) print("build_internal: %s\n", mesh.size());

    if (mesh.empty()) return pair<uint, float>(Outside, 0.0);
    ...
    // if V is too high then fallback to O(Vertices + Faces * Samples), use every face as candidate + 3 main planes
    ...
    if (vertices.size() > 10) {
        ...
        if (mesh.size() <= 10000)
            for (auto f : mesh) {
                ...
                evaluate_candidate(candidate, data, mesh, samples_begin, samples_end, best_heuristic, best_hist,
                                   best_candidate);
            }
        for (auto major_axis : range(3)) {
            ...
            for (auto v : vertices) {
                ...
                if (-vv[major_axis] == v_prev) continue;
        ...
        for (auto a : range(vertices.size()))
            for (auto b : range(a))
                for (auto c : range(b)) {
    ...
    if (best_heuristic == std::numeric_limits<long>::max()) {
        if (mesh.size() > 250) print("out of samples: %s\n", mesh.size());
    ...
    for (auto f : mesh) {
        ...
        if (ai > 0 || bi > 0 || ci > 0) pmesh.push_back(f);
        if (ai < 0 || bi < 0 || ci < 0) nmesh.push_back(f);
    }

    aabb3 box;
    for (auto iv : vertices) box.add(data.vertices[iv]);
...
void SolidBSPTree::print_tree(uint n, int depth) {
    ...
    print("plane %.3f%% [%d %d %d %d] (%s)\n", 100 * percent[n], (int)hist[n].positive, (int)hist[n].negative,
          (int)hist[n].overlap, (int)hist[n].stradle, box_size[n]);
...
bool intersects_convex(double3 v, const mesh3& mesh) {
    for (auto face : mesh) {
        ...
        if (dot(normal, v - face.a) > 0) return false;
    }
    return true;
}
...
bool intersects(double3 v, const mesh3& mesh) { return ::intersects(v, &mesh[0], &mesh[0] + mesh.size()); }
...
bool SolidBSPTree::intersects(double3 v) {
    ...
    while (true) {
        ...
        if (i == Outside) return false;
        if (i == Inside) return true;
    }
}
...
SolidBSPTree::SolidBSPTree(const mesh3& mesh, uint num_samples, std::default_random_engine& rnd) {
    ...
    tester.resize(n);
    for (auto j : range(n)) tester[j] = uniform3(data.rnd, box);
