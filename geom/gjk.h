...
inline int gjk_classify(const SupportA& support_a, const SupportB& support_b, double2& dir, uint max_iterations = 20) {
    ...
    for (auto i : range(max_iterations)) {
        ...
        if (dot(s, dir) < 0) return 1;  // disjoint
        simplex.push_back(s);
        if (gjk_simplex(simplex, dir)) return -1;  // overlapping
    }
    return 0;
}
...
inline double3 sphere_support(double3 dir, double radius) { return dir * (radius / length(dir)); }

inline double3 capsule_support(double3 dir, double size, double radius) {
    if (dir.x > 0) return double3{size, 0, 0} + dir * (radius / length(dir));
...
inline double3 box_support(double3 dir, double3 size) { return sign_no_zero(dir) * size; }
