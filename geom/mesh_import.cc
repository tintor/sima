...
mesh3 load_ply(string_view filename) {
    ...
    while (true) {
        line = file.readline();
        if (line == "") THROW(runtime_error, "bad ply file header");
        if (line == "end_header\n") break;
    ...
    while (vertices.size() < vertices.capacity()) {
        line = file.readline();
        if (line == "") THROW(runtime_error, "unexpected end of ply file");
        if (!search(line, vertex_regex, /*out*/ m)) THROW(runtime_error, "expected vertex in ply file: [%s]", line);
    ...
    while (mesh.size() < mesh.capacity()) {
        line = file.readline();
        if (line == "") THROW(runtime_error, "unexpected end of ply file");
        if (!match(line, face_regex, /*out*/ m)) THROW(runtime_error, "expected face in ply file");
        int a = parse<int>(m[1]);
        int b = parse<int>(m[3]);
        int c = parse<int>(m[5]);
