...
double3 operator"" _d3(const char* s, size_t n) {
    ...
    double3 v;
    for (int i : range<int>(3)) v[i] = parse<double>(e[i]);
    return v;
}
...
static double vdist(const vector<double3>& a, const vector<double3>& b, int shift, bool flip) {
    ...
    const int N = a.size();
    for (auto i : range(0, N)) maximize(dist, length(a[((flip ? N - 1 - i : i) + shift) % N] - b[i]));
    return dist;
}
...
void CheckPoly(const vector<double3>& expected, const vector<double3>& actual) {
    ...
    for (int shift : range<int>(0, expected.size()))
        for (bool flip : {false, true}) minimize(dist, vdist(expected, actual, shift, flip));
    REQUIRE(dist <= 1e-5);
}

void Check(double3 expected, double3 actual) {
    if (VERBOSE) print("expected %s, actual %s\n", expected, actual);
    CHECK(length(expected - actual) <= 1e-6);
}

void test(const vector<double3>& ca, const vector<double3>& cb, const int expectedResult, const double3 expectedNormal,
          const vector<double3>& expectedContacts, const double expectedOverlap) {
    ...
    if (VERBOSE) print("result %s, normal %s, contacts %s, overlap %s\n", result, normal, contacts, overlap);

    CHECK(expectedResult == result);
    if (result <= 0 && !equal(expectedNormal, double3{0, 0, 0})) Check(expectedNormal, normal);
    if (result == 0) CheckPoly(expectedContacts, contacts);
    if (result < 0) CHECK(Approx(expectedOverlap).epsilon(1e-6) == overlap);
    if (VERBOSE) print("\n");
}

vector<double3> translate(const vector<double3>& v, double3 delta) {
    vector<double3> m;
    for (double3 a : v) m.push_back(a + delta);
    return m;
}

vector<double3> scale(const vector<double3>& v, double3 scale) {
    vector<double3> m;
    for (double3 a : v) m.push_back(a * scale);
    return m;
}
...
vector<double3> rotate(const vector<double3>& v, double3 axis, double angle) {
    ...
    vector<double3> m;
    for (double3 a : v) m.push_back(quat_rotate(q, a));
    return m;
}
...
TEST_CASE("convex_body basic", "[convex_body]") {
    ...
    for (double x : {0, 1})
        for (double y : {0, 1})
            for (double z : {0, 1}) ca.push_back(double3{x, y, z});
    ...
    for (double y : {0, 1})
        for (double z : {0, 1}) ce.push_back(double3{2, y, z});
    test(ca, ce, 0, double3{0, 0, 0}, {ce[0]}, 0);

    print("vertex / face\n");
    vector<double3> cd = {double3{1, 0.5, 0.5}};
    for (double y : {0, 1})
        for (double z : {0, 1}) cd.push_back(double3{2, y, z});
    test(ca, cd, 0, double3{1, 0, 0}, {cd[0]}, 0);

    print("face / face | full face overlap\n");
    test(ca, translate(ca, double3{1, 0, 0}), 0, double3{1, 0, 0}, "1 0 0, 1 0 1, 1 1 1, 1 1 0"_vd3, 0);

    print("face / face | full face overlap with one cube rotated 45 degrees\n");
    test(ca, translate(rotate(translate(ca, double3{1, -0.5, -0.5}), double3{1, 0, 0}, PI / 4), double3{0, 0.5, 0.5}),
         0, double3{1, 0, 0},
...
Result VerifyInvariants(const vector<double3>& ca, const vector<double3>& cb) {
    ...
    if (VERBOSE) {
        print("\nA %s, B %s, result %s, normal %s, contacts %s, overlap %s\n", ma.vertices.size(), mb.vertices.size(),
              result.type, result.normal, result.contacts, result.overlap);
        auto va = ma.vertices;
        for (double3& v : va) v += ma.position;
        auto vb = mb.vertices;
        for (double3& v : vb) v += mb.position;
        print("A %s\n", va);
        print("B %s\n", vb);
    }
    REQUIRE(length(result.normal) == Approx(1).margin(1e-4));
    if (result.type == 0) REQUIRE(result.contacts.size() > 0);
    ...
    if (VERBOSE) {
        print("\nA %s, B %s, result %s, normal %s, contacts %s, overlap %s\n", ma.vertices.size(), mb.vertices.size(),
              result2.type, result2.normal, result2.contacts, result2.overlap);
        print("C1 %s\n", result.contacts);
        print("C2 %s\n", result2.contacts);
    }
...
void Generate(RND& rnd, int pos, int zero, int neg, vector<double3>& ca, int mode = 0) {
    ...
    ca.clear();
    for (auto i : range(0, pos)) ca.push_back(double3{uniform(rnd, -1, 1), uniform(rnd, -1, 1), uniform(rnd, 1e-3, 1)});
    for (auto i : range(0, zero)) {
        if (mode == 0) ca.push_back(double3{uniform(rnd, -1, 1), uniform(rnd, -1, 1), 0});
        if (mode == 1) ca.push_back(double3{uniform(rnd, -1, 1), 0, 0});
        if (mode == 2) ca.push_back(double3{0, 0, 0});
    }
