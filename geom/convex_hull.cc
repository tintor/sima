...
bool is_convex(cspan<triangle3> mesh) {
    ...
    for (auto face : mesh)
        for (auto vertex : face) vertices.push_back(double3(vertex));
    ...
    for (auto face : mesh) {
        ...
        for (auto v : vertices)
            if (dot(n, v) > d && !equal(v, face.b) && !equal(v, face.c)) return false;
    }
    return true;
}
...
// - (not needed after sort) fast SIMD scans to find extreme points (ie. points on the final hull) for 8 random
// directions
// - construct initial hull from the collected points
// - keep planes for hull faces (face can be more than triangle)
// - SIMD scan through all points (discard points already in hull, assign remaining points to single face  that they are
// in front of face with max signed distance) some vertices will be discarded, others will be paritioned
...
void convex_hull_fast(cspan<double3> points, mesh3& hull) {
    ...
    for (uint i = 0; i < points.size(); i++) order[i] = i;
...
void convex_hull(cspan<double3> points, mesh3& hull) {
    ...
    for (auto p : points) {
        ...
        if (equal(p, a) || equal(p, b) || equal(p, c) || equal(p, d)) continue;
        ...
        for (size_t i = 0; i < hull.size(); i++) {
            ...
            if (dot(normal, p - f.a) <= 0) continue;
        ...
        for (auto e : open_edges) hull.emplace_back(e.a, e.b, p);
    }
}
