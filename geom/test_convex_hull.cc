...
bool less(cspan<double3> v, size_t a, size_t b) {
    for (auto i : range(v.size())) {
        ...
        if (!equal(aa, bb)) return less(aa, bb);
    }
    return false;
};
...
static vector<double3> normalize(cspan<double3> v) {
    ...
    vector<double3> w;
    w.resize(n);
    for (auto i : range(n)) w[i] = v[(i + m) % n];
    return w;
}
...
void REQUIRE_EQ(const polygon3& a, const polygon3& b) {
    ...
    for (auto i : range(a.size())) REQUIRE(equal(a[i], b[i]));
}

void REQUIRE_EQ(const poly_mesh3& a, const poly_mesh3& b) {
    REQUIRE(a.size() == b.size());
    for (auto i : range(a.size())) REQUIRE_EQ(a[i], b[i]);
}
...
inline void format_e(string& s, string_view spec, Validity v) { format_s(s, "Validity(%s)", (int)v); }
