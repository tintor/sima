...
sphere minimal_sphere_brute_force(cspan<double3> points) {
    ...
    if (points.size() == 2) return minimal_sphere(points[0], points[1]);
    if (points.size() == 3) return minimal_sphere(points[0], points[1], points[2]);
...
double max_distance(double3 center, cspan<double3> points) {
    double d2 = 0;
    for (double3 p : points) d2 = max(d2, squared(p - center));
    return sqrt(d2);
}
...
TEST_CASE("minimal_sphere(span<double3>)", "[sphere]") {
    ...
    for (auto i : range(5000)) {
        ...
        points.resize(size);
        for (auto j : range(size)) points[j] = uniform3(rnd, -1.0, 1.0);
