...
double distance(segment2 a, double2 b) { return length(a.nearest(b) - b); }

double distance(double2 a, segment2 b) { return distance(b, a); }
...
double distance(double3 p, triangle3 m) {
    ...
    for (auto [a, b] : Edges(m))
        if (plane::sign(a, b, a + normal, p) > 0) return distance(p, segment3(a, b));
    return abs(dot(normal, p - m.a));
}

double distance(double3 v, triangle3 m, const plane& p) {
    for (auto [a, b] : Edges(m))
        if (plane::sign(a, b, a + p.normal(), v) > 0) return distance(v, segment3(a, b));
...
bool intersection(line3 e, triangle3 m, /*out*/ double3& result) {
    ...
    auto denom = u + v + w;
    if (denom < 1e-8) THROW(runtime_error, "planar case");
...
double disjoint_distance(segment3 e, triangle3 m) {
    ...
    if (intersects_in_point(e, m)) return 0.0;
    ...
    if (inside_triangle_prism(e.a, m, n)) d = min(d, abs(dot(n, e.a - m.a)));
    if (inside_triangle_prism(e.b, m, n)) d = min(d, abs(dot(n, e.b - m.a)));
    return d;
}
...
double disjoint_distance(triangle3 p, triangle3 q) {
    ...
    for (auto i : range(3)) d = min(d, distance(p, q[i]), distance(p[i], q));

    double ds = 1e100;
    for (auto ep : Edges(p))
        for (auto eq : Edges(q)) ds = min(ds, squared_distance(ep, eq));
...
double distance(triangle3 p, triangle3 q) {
    ...
    for (auto e : Edges(p))
        if (intersects_in_point(e, q)) return 0.0;
...
double edge_angle(double3 a, double3 b, double3 c, double3 d) { THROW(not_implemented); }
