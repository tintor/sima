...
inline bool ContainsSimilarAxis(const vector<double3>& v, double3 a) {
    for (double3 e : v) {
        double t = angle(e, a);
        if (t <= Tolerance || PI - t <= Tolerance) return true;
    }
    return false;
}
...
cmesh3 GenerateConvexMesh(cspan<double3> vertices) {
    ...
    for (double3& v : m.vertices) v -= m.position;
    for (auto& f : m.mesh)
        for (auto& v : f) v -= m.position;
    ...
    for (const auto& f : m.mesh) {
        ...
        if (!ContainsSimilarAxis(m.faceAxis, a)) m.faceAxis.push_back(a);
        for (auto e : Edges(f)) edges.insert({e, a});
    }
    ...
    for (const auto& p : edges) {
        ...
        if (!lex_less(e.a, e.b) && angle(edges.at(segment3{e.b, e.a}), p.second) > Tolerance) edge_list.push_back(e);
    }
    // order by edge length, longest to shortestt
    sort(edge_list, [](segment3 p, segment3 q) { return squared(p.a - p.b) > squared(q.a - q.b); });

    for (segment3 e : edge_list) {
        double3 a = normalize(e.a - e.b);
        if (!ContainsSimilarAxis(m.edgeAxis, a)) m.edgeAxis.push_back(a);
    }
    return m;
}

inline void FilterVertices(const vector<double3>& vertices, double3 axis, double dist, double3 position,
                           vector<double3>& out) {
    out.clear();
    for (double3 v : vertices) {
        double d = dot(v, axis);
        if (dist - Tolerance <= d && d <= dist + Tolerance) out.push_back(v + position);
    }
}
...
inline void ConvexHullInPlace2(vector<double3>& points) {
    ...
    for (size_t i = 1; i < points.size(); i++)
        if (points[i].y < points[leastY].y) leastY = i;
    swap(points[0], points[leastY]);

    auto ccw = [](double3 a, double3 b, double3 c) { return -((b.x - a.x) * (c.y - a.y) - (b.y - a.y) * (c.x - a.x)); };

    double3 pivot = points[0];
    sort(points.begin() + 1, points.end(), [&pivot, &ccw](double3 a, double3 b) {
        double order = ccw(pivot, a, b);
        if (order == 0) return squared(pivot - a) < squared(pivot - b);
        return order < 0;
    });
...
inline double signed_double_edge_area2(double3 a, double3 b) { return (a.x + b.x) * (a.y - b.y); }
...
inline bool PointInPolygon2(double3 p, cspan<double3> ring) {
    for (auto e : Edges(ring))
        if (Classify2(e, p) == 0) return true;
...
static void ConvexIntersect2(vector<double3>& a, vector<double3>& b, vector<double3>& c) {
    ...
    for (segment3 ea : Edges(a))
        for (segment3 eb : Edges(b))
            if (StrictIntersect2(ea.a, ea.b, eb.a, eb.b, v)) c.push_back(v);

    if (b.size() >= 3)
        for (double3 v : a)
            if (PointInPolygon2(v, b)) c.push_back(v);
    if (a.size() >= 3)
        for (double3 v : b)
            if (PointInPolygon2(v, a)) c.push_back(v);
...
static bool ProcessContacts(double iaMax, double ibMin, double pa, double pb, const cmesh3& ca, const cmesh3& cb,
                            double3& normal, vector<double3>& contacts, vector<double3>& workA,
    ...
    // TODO fast path for f/f: if all vertices of smaller face are inside edge planes of larger face just return smaller
    // face
    ...
    for (double3& v : workA) v = {dot(v, j), dot(v, k), 0};
    for (double3& v : workB) v = {dot(v, j), dot(v, k), 0};
    ...
    for (double3& v : contacts) v = solve_linear_row(j, k, normal, double3{v.x, v.y, m});
    return true;
}
...
int ClassifyConvexConvex(const cmesh3& ca, const cmesh3& cb, bool hasNormal, double3& normal, vector<double3>& contacts,
    ...
    auto check = [&](double3 axis) {
        if (hasNormal && equal(axis, initialNormal)) return false;
        // TODO it is possible to pre-compute lower and upper bounds for an object for all axises to speed up disjoint
        // check?
    ...
    for (double3 axisA : ca.edgeAxis)
        for (double3 axisB : cb.edgeAxis) {
            ...
            if (axisLen >= Tolerance)
                if (check(axis / axisLen)) return result;
        }
