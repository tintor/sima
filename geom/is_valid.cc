...
bool IsValidPolygon(cspan<double2> poly) {
    ...
    for (auto i : range(n))
        for (auto j : range(i))
            if (equal(poly[i], poly[j])) return false;
    ...
    for (auto i : range(n)) {
        ...
        for (auto j : range(i)) {
            ...
            if (relate_abxo(segment2(a, b), segment2(p, q), inv_ab_len)) return false;
        }
    }
    return true;
}

bool IsValid(const polygon2& poly) { return IsValidPolygon(poly); }

bool IsValid(const xpolygon2& poly) { THROW(not_implemented); }
...
bool edge_overlap(triangle3 p, triangle3 q) {
    if (!Intersects(aabb3(p), aabb3(q))) return false;
    for (segment3 pe : Edges(p))
        for (segment3 qe : Edges(q))
            if (colinear(pe.a, qe.a, qe.b) && colinear(pe.b, qe.a, qe.b)) return Intersects(aabb3(pe), aabb3(qe));
    return false;
}
...
bool contains_multiple_components(const mesh3& mesh) {
    ...
    for (auto i : range(mesh.size()))
        for (auto j : range(i))
            if (edge_overlap(mesh[i], mesh[j])) component[i].merge(component[j]);
    for (auto& c : component)
        if (c.find() != component[0].find()) return true;
    return false;
}
        ...
        for (triangle3 n : mesh)
            if (Intersects(aabb3(n), box))
                for (segment3 e : Edges(n)) {
                    if (equal(e.a, m.a) || equal(e.a, m.b) || equal(e.a, m.c) || equal(e.b, m.a) || equal(e.b, m.b) ||
                        equal(e.b, m.c))
                        continue;
                    double3 d = e.b - e.a;
...
bool contains_open_edge(const mesh3& mesh) {
    ...
    for (auto i : range(mesh.size()))
        for (auto e : mesh[i].edges())
            if (open_edges.erase(e.reversed()) == 0) open_edges.insert(e);
    for (segment3 e : open_edges) print("open edge: %s\n", e);
...
void format_e(string& s, string_view spec, const Point& p) {
    format_s(s, "{pos: %s, angle_pos: %s, angle_off: %s, begin: %s, ccw: %s}", p.pos, p.angle_pos, p.angle_off, p.begin,
             p.ccw);
}
...
bool IsSealed(const mesh3& mesh) {
    ...
    for (const auto& [line, data] : lines) {
        ...
        auto less = [](pair<double, bool> a, pair<double, bool> b) { return a.first < b.first; };
        ...
        for (auto a = data.points.begin(); a != data.points.end(); a++) {
            ...
            if (a->begin && b != data.points.end() && !b->begin)
                for (auto p = angles.begin(); p != angles.end(); p++) {
                    auto q = p;
                    ++q;
                    if (q == angles.end()) q = angles.begin();
...
bool contains_overlapping_faces(const mesh3& mesh) {
    ...
    for (auto i : range(mesh.size())) {
        ...
        for (auto j : range(i)) {
            auto q = mesh[j];
            if (dot(q.a - p.a, n) != 0) continue;
            if (dot(q.b - p.a, n) != 0) continue;
            if (dot(q.c - p.a, n) != 0) continue;
            if (are_coplanar_faces_overlapping(p, q, n)) return false;
        }
    }
    return false;
}
...
bool AreAllEdgesConnected(const mesh3& mesh) {
    ...
    for (const auto& f : mesh)
        for (segment3 e : Edges(f)) {
            if (edges.count(e) > 0) return false;
            edges.insert(e);
        }

    for (segment3 e : edges) print("edge %s\n", e);
...
Validity IsValid(const mesh3& mesh) {
    if (mesh.size() < 4) return Validity::TooFewFaces;

    // All faces must be valid
    for (auto f : mesh)
        if (colinear(f[0], f[1], f[2])) return Validity::InvalidFace;

    if (contains_multiple_components(mesh)) return Validity::SeparateComponents;

    if (face_cuts_into_other_face(mesh)) return Validity::SelfIntersection;

    if (contains_overlapping_faces(mesh)) return Validity::OverlappingFaces;
    ...
    if (SignedVolume(mesh) < 0) return Validity::Inverted;
...
int Classify(plane p, face f) {
    int count[3] = {0, 0, 0};
    for (double3 v : f.vertices()) count[Sign(p, v) + 1] += 1;
    int neg = count[0];
    int zero = count[1];
    int pos = count[2];
...
bool ContainsOverlappingFaces(const xmesh3& mesh) {
    ...
    for (auto i : range(mesh.size())) {
        ...
        for (auto j : range(i)) {
            face q = mesh[j];
            if (AreCoplanar(pp, q) && AreCoplanarFacesOverlapping(p, q)) return true;
        }
    }
    return false;
}
...
bool IsSealed(const xmesh3& mesh) {
    ...
    for (face f : mesh)
        for (auto ring : f)
            for (auto edge : Edges(ring)) edges.insert(edge);
    for (const segment3& e : edges)
        if (edges.count(e) != edges.count(e.reversed())) return false;
    return true;
}
...
Validity IsValid(const xmesh3& mesh) {
    ...
    // All faces must be valid
    for (face f : mesh) {
        if (!AreCoplanar(best_fit_plane(f.vertices()), f)) return Validity::NonPlanarFace;
        xpolygon2 poly = Project(f, ProjectionAxis(f));
        if (!IsValid(poly)) return Validity::InvalidFace;
    }
    ...
    if (ContainsOverlappingFaces(mesh)) return Validity::OverlappingFaces;

    if (!IsSealed(mesh)) return Validity::NotSealed;
...
void MakeValid(mesh3& mesh) { THROW(not_implemented); }

void MakeValid(xmesh3& mesh) { THROW(not_implemented); }
