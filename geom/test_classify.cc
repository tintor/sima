...
TEST_CASE("Sign(segment2, double2)", "[classify]") {
    ...
    for (int i = 0; i < 10000; i++) {
        ...
        for (int j = 0; j < 100; j++) REQUIRE(Sign(s, s.linear(uniform(rnd, -2, 3))) == 0);
    }
}
...
double2 swap(double2 a) { return double2{a.y, a.x}; }

double distanceRingPoint(cspan<double2> ring, double2 p) {
    double dist = std::numeric_limits<double>::max();
    for (const segment2& e : Edges(ring)) dist = min(dist, distance(e, p));
    return dist;
}
...
TEST_CASE("Classify(xpolygon2, double2) random triangle", "[classify]") {
    ...
    for (int i = 0; i < 10000; i++) {
        double2 a = uniform2(rnd, -10, 10);
        double2 b = uniform2(rnd, -10, 10);
        while (Equals(a, b)) b = uniform2(rnd, -10, 10);
        double2 c = uniform2(rnd, -10, 10);
        while (Equals(a, c) || Equals(b, c)) c = uniform2(rnd, -10, 10);
...
long squared(int2 a) { return (long)a.x * a.x + (long)a.y * a.y; }
...
static polygon2 random_polygon(int size, RNG& rng) {
    ...
    while (poly.size() < size) {
        ...
        if (All(poly, [p](double2 a) { return !Equals(a, p); })) poly.push_back(p);
    }
    tsp_sort(poly);
    return poly;
}
...
void transform(vector<double2>& poly, int t) {
    for (double2& p : poly) p = transform(p, t);
    if (t & 8)
        for (int i = 0; i < poly.size() / 2; i++) swap(poly[i], poly[poly.size() - 1 - i]);
}
...
TEST_CASE("relate(segment2, segment2)", "[classify]") {
    ...
    for (int i = 0; i < 300000; i++) {
        ...
        while (Equals(a, d) || Equals(b, d) || Equals(c, d)) d = uniform2(rnd, -10, 10);
...
TEST_CASE("Classify(polygon2, segment2) random polyogn", "[classify]") {
    ...
    for (int i = 0; i < 500; i++) {
        ...
        for (auto j : range<size_t>(1, p.size() - 1)) tests.emplace_back(segment2(p[0], p[j]), 9);
...
polygon2 MakeTriangle(double x, double y) { return polygon2{double2{0, y}, double2{-x / 2, 0}, double2{x / 2, 0}}; }
...
polygon2 MakeL(double x, double y, double a) {
    return polygon2{double2{-x, -y},       double2{x, -y},    double2{x, a - y},
                    double2{a - x, a - y}, double2{a - x, y}, double2{-x, y}};
}

double TransformUntilContact(polygon2& a, const polygon2& b, std::function<void(polygon2&, double)> transform,
                             double inc) {
    double t = 0;
    polygon2 orig = a;
    while (true) {
...
double RotateUntilContact(polygon2& a, const polygon2& b, double t) {
    return TransformUntilContact(
        a, b, [t](polygon2& m, double a) { Rotate(m, t); }, PI / 72);
}

double TranslateUntilContact(polygon2& a, const polygon2& b, double2 t) {
    return TransformUntilContact(
        a, b, [t](polygon2& m, double a) { Translate(m, t * a); }, 1);
}

double ScaleUntilContact(polygon2& a, const polygon2& b, double t) {
    return TransformUntilContact(
        a, b, [t](polygon2& m, double a) { Scale(m, 1 + t); }, 0.001);
}
...
TEST_CASE("Classify(polygon2, polygon2) small box, large box", "[classify2]") {
    ...
    for (const auto& c : contacts) print("sa %s, sb %s, normal %s\n", c.sa, c.sb, c.normal);
}
