...
inline quat quat_from_to(double3 a, double3 b) {
    auto axis = cross(a, b);
    if (squared(axis) < 1e-12) axis = any_normal(a);
    return quat_from_axis_angle(axis, acos(dot(a, b) / sqrt(squared(a) * squared(b))));
}

inline quat quat_from_x_axis(double angle) { return {sin(angle / 2), 0, 0, cos(angle / 2)}; }

inline quat quat_from_y_axis(double angle) { return {0, sin(angle / 2), 0, cos(angle / 2)}; }

inline quat quat_from_z_axis(double angle) { return {0, 0, sin(angle / 2), cos(angle / 2)}; }
...
inline quat quat_inv(quat a) { return {-a.x, -a.y, -a.z, a.w}; }
...
// note: not normalized!
inline double3 quat_axis(quat q) { return {q.x, q.y, q.z}; }
...
inline double quat_dot(quat a, quat b) { return a.w * b.w + a.x * b.x + a.y * b.y + a.z * b.z; }
...
inline quat lerp(quat p, quat q, double t) { return p * (1 - t) + q * t; }
...
inline quat quat_from_matrix(double33 m) {
    ...
    if (m.c.y > m.b.z) q.x = -q.x;
    if (m.a.z > m.c.x) q.y = -q.y;
    if (m.b.x > m.a.y) q.z = -q.z;
    return q;
}
