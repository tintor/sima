...
T ShardedSum2(cspan<float> v) {
    auto n = v.size() / 8;
    return NaiveSum<T>(v.subspan(n*0, n)) +
           NaiveSum<T>(v.subspan(n*1, n)) +
           NaiveSum<T>(v.subspan(n*2, n)) +
           NaiveSum<T>(v.subspan(n*3, n)) +
           NaiveSum<T>(v.subspan(n*4, n)) +
           NaiveSum<T>(v.subspan(n*5, n)) +
           NaiveSum<T>(v.subspan(n*6, n)) +
           NaiveSum<T>(v.subspan(n*7, n));
}
...
TEST_CASE("sum", "[util][.]") {
    ...
    vector<string> rows;
    for (auto i : range(func.size()))
        rows << format("%s|%.5f|%.5f|%f|%f", name[i], acc[i].mean(), acc[i].max() - acc[i].min(), sqrt(acc[i].variance()), ticks[i] / tm);
    PrintTable(rows, '|', " ");
    println();
    }
}
