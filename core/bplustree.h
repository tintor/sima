...
T quick_select(span<T> m, size_t kth) {
	...
	if (kth < index)
		return quick_select(m.subspan(0, index), kth);
	if (kth > index)
		return quick_select(m.subspan(index + 1), kth - index - 1);
	return *it;
}
...
private:
	void debug(branch* b, uint level) {
		...
		print("*[%s] ", b->sizes[0]);
		for (auto i : range(b->size - 1))
			print("%s[%s] ", b->keys[i], b->sizes[i+1]);
		print("\n");
		for (auto i : range(b->size))
			debug(b->pointers[i], level + 1);
	}
	...
	void insert_into_branch(uint a_size, uint b_size, branch* b, uint level, K key, const array<pair<branch*, uint>, 30>& stack) {
		if (level == 0) {
			// tree grows one level
			_branch_levels += 1;
			auto a = _root;
