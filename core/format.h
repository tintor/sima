...
#define FORMAT_MAP(MAP)                                                                    \
    ...
    inline void format_e(string& s, string_view spec, const MAP<Key, Value, Compare>& m) { \
        format_map(s, spec, m);                                                            \
    }
...
#define FORMAT_SET(SET)                                                             \
    ...
    inline void format_e(string& s, string_view spec, const SET<Key, Compare>& m) { \
        format_set(s, spec, m);                                                     \
    }
...
inline bool try_short(string& s, ucent v, ulong m, char c) {
    constexpr int E = 100;
    if (v < m - m / E) return false;

    auto a = (v + m / 2) / m;
    if (abs(a * m, v) * E > v) return false;
...
void format_int(string& s, string_view spec, Integer v) {
    if (spec == "%h"sv && v != -v) {
        ...
        if (try_short(s, v, 1000000000000lu, 't')) return;
        if (try_short(s, v, 1000000000lu, 'g')) return;
        if (try_short(s, v, 1000000, 'm')) return;
        if (try_short(s, v, 1000, 'k')) return;
    ...
    if ((spec == "%t"sv || spec == "%Y"sv) && v != -v) {
        if (spec == "%Y") v = round(v * (Timestamp::ms_per_tick() * 1e-3));
...
#define FORMAT_INT(T) \
    inline void format_e(string& s, string_view spec, const T& v) { format_int(s, spec, v); }
...
template <typename T>
inline void format_e(string& s, string_view spec, const T* v) {
    format_int(s, "%x", reinterpret_cast<size_t>(v));
}
...
inline void format_fp(string& s, double fvalue, int min, int max, bool plus, bool minus, bool zero, bool space) {
    ...
    if (minus) padlen = -padlen; /* Left Justifty */
    if (zero && (padlen > 0)) {
        if (signvalue) {
            s += signvalue;
            --padlen;
    ...
    if (signvalue) s += signvalue;
    while (iplace > 0) s += iconvert[--iplace];

    // Decimal point.
    if (max > 0) {
        s += '.';
        while (fplace > 0) s += fconvert[--fplace];
    }
    while (zpadlen > 0) {
        s += '0';
        --zpadlen;
...
inline bool isIntSpec(string_view spec) {
    return spec.size() >= 2 && spec[0] == '%' &&
           (spec.back() == 'd' || spec.back() == 'b' || spec.back() == 'x' || spec.back() == 'b' || spec.back() == 't');
}
...
inline void format_e(string& s, string_view spec, float v) {
    ...
    if (length <= 0) throw std::runtime_error("format: snprintf failed");
...
void format_a(string& s, int skip, string_view spec, const T& arg, const Args&... args) {
    if (skip == 0) {
        format_e(s, spec, arg);
    } else {
