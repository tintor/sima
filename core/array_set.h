...
class open_addressing_hashtable {
   public:
    ...
    // never updates
    bool insert(T a) { throw "not implemented"; }
    ...
    void clear() {
        assert(consistent());
        if (_size > 0) {
            for (auto i : range(_mask + 1)) clear(i);
            _size = 0;
        }
        assert(consistent());
    }
    ...
    void print() const {
        std::cout << "[";
        for (auto i : range(_mask + 1)) std::cout << " " << _array[i];
        std::cout << "]" << std::endl;
    }
   ...
   private:
    ...
    template <typename Key>
    bool equals(uint i, Key a) const {
        return _array[i].equals(a);
    }
    ...
    bool consistent() const {
        ...
        for (auto i : range(_mask + 1))
            if (!empty(i)) assert(internal_contains(_array[i]));
        // no duplicates allowed
        if (_size > 1) {
            raw_array<T> copy(_size);
            uint w = 0;
            for (auto i : range(_mask + 1))
                if (!empty(i)) copy[w++] = _array[i];
            std::sort(copy.data(), copy.data() + _size);
            for (auto i : range(_size - 1)) assert(!copy[i].equals(copy[i + 1]));
        }
        return true;
    }

    static bool between(uint a, uint b, uint c) {
        if (a <= b) return b <= c || c <= a;
        return b <= c && c <= a;
    }
    ...
    void fill_gap(uint a, uint b) {
        ...
        while (!empty((a - 1) & _mask)) a = (a - 1) & _mask;
...
class array_map {
   ...
   public:
    ...
    bool contains(T a) const { return _map.contains(a); }

    bool insert(T a, Value v) { return _map.insert({a, v}); }

    void insert_unsafe(T a, Value v) { return _map.insert_unsafe({a, v}); }

    bool erase(T a) { return _map.erase(a); }

    void erase_unsafe(T a) { return _map.erase_unsafe(a); }

    void reserve(uint size) { _map.reserve(size); }

    void shrink() { _map.shrink(); }
    ...
    iterator begin() { return iterator(_map._begin()); }

    iterator end() { return iterator(_map._end()); }

    uint max_probes() { return _map.max_probes(); }

    void print() { _map.print(); }
...
struct hash<typename ::array_map<T, Value>::Slot> {
    size_t operator()(const array_map<T, Value>::Slot& s) const { return hash<T>()(s.key); }
};
...
class array_set {
   ...
   public:
    ...
    bool contains(T a) const { return _map.get(a) != nullptr; }

    bool insert(T a) { return _map.insert(Slot{a}); }

    void insert_unsafe(T a) { return _map.insert_unsafe(Slot{a}); }
