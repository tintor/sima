...
struct Model {
	...
	bool is_valid_move(pose2 src, pose2 dest) {
		...
		for (int i = 0; i <= N; i++)
			if (collision(interpolate(src, dest, double(i) / N)))
				return false;
		return true;
	}
	...
	double start_dist(pose2 a) const {
		...
		while (a != start) {
			...
			a = i.prev;
			if (++z >= 1000000)
				THROW(runtime_error, "graph loop");
		}
		return dist;
	}
	...
	void improve_graph(pose2 p) {
		...
		for (auto it = graph.begin(); it != graph.end(); it++) {
			...
			if (dist_p_m < MaxDist && i.dist > 0
					&& dist_s_p + dist_p_m < start_dist(i.prev) + i.dist
					&& is_valid_move(p, m)) {
				i.prev = p;
				i.dist = dist_p_m;
			}
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
	...
	if (action == GLFW_PRESS && key == GLFW_KEY_LEFT)
		if (model.step > 0)
			model.step -= 1;
	if (action == GLFW_PRESS && key == GLFW_KEY_LEFT)
		if (model.step + 1 < model.solution.size())
			model.step += 1;
...
void render_piano(uint64_t color, const mat3& ortho, pose2 pose,
...
int main(int argc, char** argv) {
	...
	RunEventLoop(window, [&]() {
		...
		mat3 transform(ortho);
		transformUniform = transform;
		for (const auto& obs : model.obstacles)
			buffer.add(obs, 0xFFFFFFFF);
		...
		if (model.solution.size() >= 2) {
			...
			while (s + 1 < sd.size() && sd[s + 1] <= model.step_dist)
				s += 1;
