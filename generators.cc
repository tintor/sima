...
void print_volume(int e, span<const double3> vertex, span<const Edge> edges) {
	double volume = 0;
...
mesh3 generate_regular_polyhedra(span<const Edge> edges) {
	...
	print_volume(0, span<const double3>(vertex, count), edges);
	for (auto e : range(100)) {
		...
		for (auto i : range(count))
			vertex[i] += delta[i];
		print_volume(e + 1, span<const double3>(vertex, count), edges);
	}
...
double print_volume2(int e, span<const double3> vertex, unordered_map<pair<int, int>, double>& edges) {
	double loss = 0;
...
mesh3 generate_regular_polyhedra2(span<const pair<int, int>> faces) {
	...
	std::unordered_map<pair<int, int>, double> edges;
	count = 0;
	for (auto f : faces) {
	...
	while(true) {
		...
		print_volume2(0, span<const double3>(vertex, count), edges);
		for (auto e : range(200000)) {
			...
			for (auto i : range(count))
				vertex[i] += delta[i];
			print_volume2(e + 1, span<const double3>(vertex, count), edges);
		}
		break;
	}
	return mesh3();
