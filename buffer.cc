...
static void GenerateCircularArc(vector<double2>& arc, double2 center, double start_angle, double end_angle, double radius, int count, bool end) {
	...
	if (end)
		arc.push_back(center + double2{cos(end_angle), sin(end_angle)} * radius);
}

// returns angle in range [-M_PI, M_PI]
static double angle(double2 v) {
...
vector<double2> ComputeBuffer(span<const double2> convex, double radius, int circle_vertices) {
	...
	if (convex.size() == 1) {
		GenerateCircularArc(result, convex[0], 0, 2 * M_PI, radius, circle_vertices, false);
		return result;
	}

	// Assume the polygon is convex and counter clockwise
	const double k = circle_vertices / (2 * M_PI);
