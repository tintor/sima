...
void Apply(System& system) {
    for (auto& body : system.bodies) body->present = body->future;
}

inline double3 Average(cspan<double3> p) {
    double3 sum = {0, 0, 0};
    for (double3 a : p) sum += a;
    return sum / p.size();
}
...
bool Resolve(const Contact& contact) {
    ...
    if (dot(va, n) <= dot(vb, n)) return false;
...
void ResolveConstraints(System& system) {
        ...
        for (auto bi : range(ai + 1, system.bodies.size())) {
            ...
            for (auto& sa : a.shapes)
                for (auto& sb : b.shapes) {
                    ...
                    int result = ClassifyConvexConvex(sa.mesh, sb.mesh, false, contact.normal, contact.points, overlap,
                                                      work1, work2);
                    if (result == 0) {
                        contact.bodyA = &a;
                        contact.bodyB = &b;
                    ...
                    if (result < 0) throw "objects penetrating";
                }
        }
    ...
    for (size_t iter = 0; iter < 10; iter++) {
        ...
        for (Contact& contact : contacts)
            if (Resolve(contact)) updated = true;
        if (!updated) {
            if (iter == 0) throw "resolveCollisions(): No update in the first iteration";
            break;
        }
    }
}
