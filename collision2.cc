...
bool shallow_collision(
	...
	if (squared(pose_a.position - pose_b.position) > r * r)
		return false;
...
double2 support(double2 dir, const Shape2& shape) {
	THROW(not_implemented);
}
...
bool temporal_collision(
	...
	}, [](double2 d) { return double2{0, 0}; }, dir, 20) >= 0;
}
	...
	for (auto i : range(n)) {
		...
		if (temporal_collision(shape_a, shape_b, start_a, start_b, end_a, end_b, min_time, max_time, max_depth)) {
			...
			while (max_time - min_time > 1e-3) {
				...
				if (temporal_collision(shape_a, shape_b, start_a, start_b, end_a, end_b, min_time, mid_time, max_depth)) {
					max_time = mid_time;
				} else {
					min_time = mid_time;
				}
